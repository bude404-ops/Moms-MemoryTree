...
export default function App() {
  ...
  return <div className="min-h-screen text-stone-900">
    ...
    <main className="mx-auto max-w-6xl px-4 py-5 pb-28 md:pb-8">
      {!onboarding.state.configured && <div className="mb-5 rounded-3xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-950"><b>Development mode:</b> no production cloud provider is configured yet, so the UI uses local seed data. Provider-neutral service contracts, authorization gates, and private storage architecture are included for real deployment.</div>}
      <OnboardingGate state={onboarding.state} onSignIn={onboarding.actions.signIn} onSignUp={onboarding.actions.signUp} onRequestPasswordReset={(email) => onboarding.actions.requestPasswordReset({ email })} onUpdatePassword={(password) => onboarding.actions.updatePassword({ password })} onCreateFamily={onboarding.actions.createFirstFamily} onAcceptInvitation={onboarding.actions.acceptInvitation} />
